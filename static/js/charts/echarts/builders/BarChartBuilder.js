...
export function buildBarSeries({ setsT, setsA, setsM, setsC, centers, interval, stepMs, labels, colorMap, providerRows, providerKey }) {
  ...
  // ASR with Heatmap
  list.push({
    id: 'as', name: 'ASR', type: 'bar', xAxisIndex: 1, yAxisIndex: 1, large: true, largeThreshold: 300, label: { show: false, formatter: (item) => { if (item.value === undefined || item.value === null || item.value === '') { return ''; } return Number(item.value).toFixed(1); } }, barWidth: bw, barGap: '4%', barCategoryGap: '20%', emphasis: { focus: 'series', blurScope: 'coordinateSystem' }, blur: { itemStyle: { opacity: 0.12 } },
    itemStyle: {
      color: (params) => {
        const c = getHeatmapColor('ASR', params.value);
        return c || colors.ASR;
      }
    },
    data: setsA.curr
  ...
  list.push({ id: 'mn', name: 'Minutes', type: 'bar', xAxisIndex: 2, yAxisIndex: 2, large: true, largeThreshold: 300, label: { show: false, formatter: (item) => { if (item.value === undefined || item.value === null || item.value === '') { return ''; } return Number(item.value).toFixed(1); } }, barWidth: bw, barGap: '4%', barCategoryGap: '20%', emphasis: { focus: 'series', blurScope: 'coordinateSystem' }, blur: { itemStyle: { opacity: 0.12 } }, itemStyle: { color: colors.Minutes }, data: setsM.curr });
  list.push({ id: 'mnPrev', name: 'Minutes -24h', type: 'bar', xAxisIndex: 2, yAxisIndex: 2, large: true, largeThreshold: 300, label: { show: false, formatter: (item) => { if (item.value === undefined || item.value === null || item.value === '') { return ''; } return Number(item.value).toFixed(1); } }, barWidth: bw, barGap: '4%', barCategoryGap: '20%', itemStyle: { color: 'rgba(140,148,156,0.85)' }, data: setsM.prev, emphasis: { disabled: true }, tooltip: { show: false }, silent: true, blur: { itemStyle: { opacity: 0.4 } } });

  // ACD with Heatmap
  list.push({
    id: 'ac', name: 'ACD', type: 'bar', xAxisIndex: 3, yAxisIndex: 3, large: true, largeThreshold: 300, label: { show: false, formatter: (item) => { if (item.value === undefined || item.value === null || item.value === '') { return ''; } return Number(item.value).toFixed(1); } }, barWidth: bw, barGap: '4%', barCategoryGap: '20%', emphasis: { focus: 'series', blurScope: 'coordinateSystem' }, blur: { itemStyle: { opacity: 0.12 } },
    itemStyle: {
      color: (params) => {
        const c = getHeatmapColor('ACD', params.value);
        return c || colors.ACD;
      }
    },
    data: setsC.curr
  ...
  list.push({ id: 'acPrev', name: 'ACD -24h', type: 'bar', xAxisIndex: 3, yAxisIndex: 3, large: true, largeThreshold: 300, label: { show: false, formatter: (item) => { if (item.value === undefined || item.value === null || item.value === '') { return ''; } return Number(item.value).toFixed(1); } }, barWidth: bw, barGap: '4%', barCategoryGap: '20%', itemStyle: { color: 'rgba(140,148,156,0.85)' }, data: setsC.prev, emphasis: { disabled: true }, tooltip: { show: false }, silent: true, blur: { itemStyle: { opacity: 0.4 } } });
