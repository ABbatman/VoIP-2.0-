export function makeStreamTooltip(state, ctx = {}) {
  ...
  return {
    ...
    formatter: (params) => {
      ...
      // Prefer the actual hovered series captured by chart mouseover
      ...
      try {
        const hoveredName = (typeof window !== 'undefined') ? window.__streamHoveredSeriesName : null;
      ...
      if (!hoveredParam && chart) {
        try {
          ...
          const seriesOpt = Array.isArray(opt.series) ? opt.series : [];
          // Pixel -> data Y
          let mouseX = (typeof window !== 'undefined') ? Number(window.__streamMouseX) : NaN;
          let mouseY = (typeof window !== 'undefined') ? Number(window.__streamMouseY) : NaN;
          let yData = null;
          if (Number.isFinite(mouseX) && Number.isFinite(mouseY)) {
            const pt = chart.convertFromPixel({ xAxisIndex: 0, yAxisIndex: 0 }, [mouseX, mouseY]);
          ...
          for (const p of ordered) {
            ...
            const add = Number.isFinite(y) ? Math.max(0, y) : 0;
            const next = acc + add;
            if (yData != null && Number.isFinite(yData)) {
              if (yData <= next) { hoveredParam = p; break; }
            }
            acc = next;
          }
        } catch(_) { /* ignore */ }
      ...
      if (!hoveredParam) {
        for (let i = arr.length - 1; i >= 0; i--) {
          const p = arr[i];
          if (p && p.value && p.value[1] != null && Number.isFinite(p.value[1])) { hoveredParam = p; break; }
        }
      }
      if (!hoveredParam) hoveredParam = arr[0];
      if (!hoveredParam || !hoveredParam.value) return '';

      // Resolve timestamp from axisValue to match grid x
      let ts = Number(arr[0]?.axisValue);
      if (!Number.isFinite(ts)) ts = Number(hoveredParam?.value?.[0]);
      const time = new Date(ts);
      const timeStr = time.toLocaleString('ru-RU', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
      let val = hoveredParam && hoveredParam.value ? hoveredParam.value[1] : null;
      // If value is missing at this x for hovered series, resolve from chart option
      if ((val == null || !Number.isFinite(val)) && chart && (Number.isFinite(hoveredIdx) || hoveredParam?.seriesName)) {
        try {
          const opt = chart.getOption();
          const ser = Number.isFinite(hoveredIdx) ? (opt.series || [])[hoveredIdx] : (opt.series || []).find(s => s && s.name === hoveredParam.seriesName);
          const data = Array.isArray(ser?.data) ? ser.data : [];
          // Find exact match by ts; if not, nearest within half-step
          let y = null;
          let step = null;
          if (data.length >= 2) {
            const x0 = Array.isArray(data[0]) ? Number(data[0][0]) : Number(data[0]?.value?.[0]);
            const x1 = Array.isArray(data[1]) ? Number(data[1][0]) : Number(data[1]?.value?.[0]);
            if (Number.isFinite(x0) && Number.isFinite(x1)) step = Math.abs(x1 - x0);
          }
          for (const d of data) {
            const x = Array.isArray(d) ? Number(d[0]) : Number(d?.value?.[0]);
            const v = Array.isArray(d) ? d[1] : (d?.value?.[1]);
      ...
      let groupBy = 'destination';
      if (!isAll(state.destination) || !isAll(state.supplier)) groupBy = 'customer';
      else if (!isAll(state.customer)) groupBy = 'destination';
      else groupBy = 'customer';
      // Build metadata lines reflecting the hovered layer and selected filters
      const lines = [];
      if (groupBy === 'customer') {
        lines.push(`Customer: ${hoveredParam.seriesName}`);
        if (!isAll(state.supplier)) lines.push(`Supplier: ${state.supplier}`);
        if (!isAll(state.destination)) lines.push(`Direction: ${state.destination}`);
      } else if (groupBy === 'destination') {
        lines.push(`Direction: ${hoveredParam.seriesName}`);
        if (!isAll(state.customer)) lines.push(`Customer: ${state.customer}`);
        if (!isAll(state.supplier)) lines.push(`Supplier: ${state.supplier}`);
      }
      if (lines.length) {
        html += `<div style="margin-top: 4px; margin-left: 16px; font-size: 12px; color: #6b7280;">${lines.join(' | ')}</div>`;
      }
      return html;
    }
  };
