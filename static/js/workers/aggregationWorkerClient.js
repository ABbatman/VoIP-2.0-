...
export function aggregatePeerRowsSync(hourlyRows) {
  ...
  for (const row of hourlyRows) {
    ...
    g.YMin += num(row.YMin); g.YSCall += int(row.YSCall); g.YTCall += int(row.YTCall);
  ...
  for (const g of groups.values()) {
    ...
    result.push({
      ...
      Min_delta: round1(pct(g.Min, g.YMin)), ACD_delta: round1(pct(ACD, YACD)),
      ASR_delta: round1(pct(ASR, YASR)), SCall_delta: round1(pct(g.SCall, g.YSCall)),
